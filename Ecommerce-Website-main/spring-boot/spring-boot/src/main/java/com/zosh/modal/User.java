...
public class User {
    ...
    //one users has many address thats why one to many
    //cascade type used when delete user then delete address also
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<Address> addresses=new ArrayList<>();

    //paymentinformation is not entity thats why give @Embedded
    @Embedded
    //for create separate table
    @ElementCollection
    @CollectionTable(name="payment_information",joinColumns = @JoinColumn(name="user_id"))
    private List<PaymentInformation> paymentInformation=new ArrayList<>();

    
    //one user give rating to multiple products
    //mappedBy for do not create separate table
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL,orphanRemoval = true)
    @JsonIgnore
